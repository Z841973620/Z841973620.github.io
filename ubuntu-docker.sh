...
if [ $DGX_REPO == true ]; then	
	if [[ $UBUNTU_CODENAME = jammy ]] || [[ $UBUNTU_CODENAME = focal ]]; then
		...
		dpkg -i dgx-repo_1.0-5_amd64.deb nvidia-repo-keys_22.04-1_all.deb
		rm dgx-repo_1.0-5_amd64.deb nvidia-repo-keys_22.04-1_all.deb
	else
		if [[ $UBUNTU_CODENAME = xenial ]]; then
			...
			dpkg -i dgx1-repo-ubuntu1404_1.1-1_amd64.deb
			rm dgx1-repo-ubuntu1404_1.1-1_amd64.deb
		else
...
if cat /etc/apt/sources.list.d/dgx.list > /dev/null; then
	...
	elif [[ $UBUNTU_CODENAME = bionic ]] || [[ $UBUNTU_CODENAME = xenial ]] || [[ $UBUNTU_CODENAME = trusty ]]; then
